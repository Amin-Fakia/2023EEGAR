...
mesh.compute_quality().cmap('jet', input_array=intrp, on="points", vmin=-vmax, vmax=vmax).add_scalarbar(pos=(0.8,0.3))
...
def slider1(widget, event):
    ...
    mesh.compute_quality().cmap('jet', input_array=intpr, on="points", vmin=-vmax, vmax=vmax)
    global colors
    global win_idx
    win_idx = value
    colors = getRGB(mesh).tolist()
plt.add_slider(slider1, 0, len(data[0])-1, value=0,
